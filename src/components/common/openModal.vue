  ...
  <div :class="overlay_classes" v-show="is_open" v-on:click="_onOverlayClick">
    <div :class="modal_classes" :style="modal_style">
      <div class="open-box-actions">
        ...
        <!-- Close Button -->
        <div class="open-action-close" v-on:click="close" v-if="!hideCloseButton">
      ...
      <div class="open-title" v-if="has_title || has_tabs">
        <!-- Tabs but no title -->
        <template v-if="has_tabs && !has_title">
          <ul class="open-modal-tabs">
            <li v-for="(tab, index) in tabs"  :key='index' :class="_getClassesForTab(tab)">
              <a href="#" v-on:click.prevent="_changeTab(tab)">
                <div class="open-modal-valign">
                  <span v-if="tab.icon" v-html="tab.icon" class="open-modal-tab-icon" />
                  <span class="open-modal-tab-title">{{ tab.title }}</span>
                </div>
              </a>
            </li>
          </ul>
      ...
      <ul class="open-modal-tabs" v-if="has_title && has_tabs">
        <li v-for="(tab, index) in tabs"  :key='index' :class="_getClassesForTab(tab)">
          <a href="#" v-on:click.prevent="_changeTab(tab)">
            <div class="open-modal-valign">
              <span v-if="tab.icon" v-html="tab.icon" class="open-modal-tab-icon" />
      ...
      <!-- Content: Wrapper -->
      <div class="open-content" ref="content">
        <!-- Icon: Error -->
        <div class="open-modal-icon open-modal-error" v-if="icon === 'error'" ref="icon_error">
          <span class="open-modal-x-mark">
            <span class="open-modal-line open-modal-left"></span>
            <span class="open-modal-line open-modal-right"></span>
          </span>
        </div>

        <!-- Icon: Warning -->
        <div class="open-modal-icon open-modal-warning" v-if="icon === 'warning'" ref="icon_warning">
          <span class="open-modal-body"></span>
          <span class="open-modal-dot"></span>
        </div>

        <!-- Icon: Info -->
        <div class="open-modal-icon open-modal-info" v-if="icon === 'info'" ref="icon_info"></div>

        <!-- Icon: Success -->
        <div class="open-modal-icon open-modal-success" v-if="icon === 'success'" ref="icon_success">
          <span class="open-modal-line open-modal-tip"></span>
          <span class="open-modal-line open-modal-long"></span>
          <div class="open-modal-placeholder"></div>
          <div class="open-modal-fix"></div>
        </div>

        <!-- Actual Content -->
        <div class="open-content-content" v-if="$slots.default">
          <slot></slot>
        </div>
      </div>

      <!-- Buttons -->
      <div class="open-buttons" v-if="$slots.button">
  ...
  methods: {
    ...
    open (tabId = null) {
      if (tabId && this.has_tabs) {
        ...
        const openingTabs = this.tabs.filter((tab) => { return tab.id === tabId })
        if (openingTabs.length > 0) {
          ...
          const openingTab = this.tabs[tabId]
          if (openingTab) {
    ...
    _onOverlayClick (event) {
      if (!event.target.classList || event.target.classList.contains('open-modal-clickable')) {
        if (this.blocking) {
      ...
      switch (this.icon) {
        case 'success':
          setTimeout(() => {
            this._applyClasses(this.$refs.icon_success, {
              '': ['animate'],
              '.open-modal-tip': ['animateSuccessTip'],
              '.open-modal-long': ['animateSuccessLong']
            })
          }, 80)
        ...
        case 'warning':
          this._applyClasses(this.$refs.icon_warning, {
            '': ['pulseWarning'],
            '.open-modal-body': ['pulseWarningIns'],
            '.open-modal-dot': ['pulseWarningIns']
          })
        ...
        case 'error':
          setTimeout(() => {
            this._applyClasses(this.$refs.icon_error, {
              '': ['animateErrorIcon'],
              '.open-modal-x-mark': ['animateXMark']
            })
          }, 80)
...
<style lang="scss" scoped>
  @import '../../styles/component/modal/mixins.scss';
  @import '../../styles/component/modal/colors.scss';
  @import '../../styles/component/modal/animations.scss';
  @import '../../styles/component/modal/icons.scss';
  ...
  .open-modal {
    ...
    ul.open-modal-tabs {
      ...
      li.open-modal-tab {
        ...
        a {
          ...
          span.open-modal-tab-title {
            display: block;
          }

          span.open-modal-tab-icon {
            display: block;
            line-height: 1.0;
          ...
          span.open-modal-tab-icon + span.open-modal-tab-title {
            line-height: 1.0;
            margin-top: 8px;
          }
        }
    ...
    &.has-tabs.has-icon .open-content {
      padding: {
        top: 32px;
        bottom: 32px;
      }
